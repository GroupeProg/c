...
image supLine(image pimg){
    ...
    image *cpimg = (image *)malloc(sizeof(image) * 2); //on alloue de la memoire pour la nouvelle image cpimg.

    assert( cpimg != NULL ); //on teste si le malloc a bien pu alloer la memoire demandee.
...
int writeLine(FILE *fichier, image pimg){
    ...
    for(int i = 0; i < X; i++){ //on parcoure autant de fois qu'il y a de pixels dans la premiere ligne.
        pixel pix = pimg.img[i]; //on cree un pixel pix qui contient le pixel de la case i.
...
pixel init_pixel(int rouge, int vert, int bleu){
...
pixel random_pixel(){
    ...
    int rouge = tab_of_colors[0]; //donc on cree un entier rouge pour y mettre la premiere valeur du tableau.
    int vert = tab_of_colors[1]; //on cree un entier vert pour y mettre la deuxieme valeur du tableau.
    int bleu = tab_of_colors[2]; //on cree un entier bleu pour y mettre la troisieme valeur du tableau.
...
image init_image(int sx, int sy){
    ...
    img1.img = (pixel *)malloc(sx * sy * sizeof(pixel) * 2); //on alloue de la memoire pour le tableau de pixels de img1.
    assert( img1.img != NULL ); //on test si la memoire a bien ete alloue.

    for(int i = 0; i < len ; i++){ //on itere du nombre de pixels de l'image.

        pixel actpix = random_pixel(); //on cree un pixel actpix et lui affecte un pixel avec des valeurs aleatoires.
        img1.img[i] = actpix; //on affecte au pixel de la case i du tableau de pixels de img1, le pixel actpix.
    }
